...
1. Member 로 순수 자바 객체 member가 메모리에 생성(비영속상태)
2. em.persist(member)를 통해 member 객체가 JPA 영속성 컨텍스트 관리(영속 상태)
3. 1차 캐시에 저장 : member 객체의 id를 키, 객체 자체를 값으로 젖아
4. SQL 생성 + 지연 : JPA는 DB에 저장하기 위한 Insert 문을 쓰기 저장소에 모아둠

### @ManyToOne

1. @Entity 어노테이션이 붙은 클래스 찾기

2. 각 엔티티 클래스를 읽어서 @Id, @Column 등 모든 매핑 어노테이션 정보 분석

3. MetaModel을 메모리에 구축(Member 클래스는 MEMBER 테이블에 해당하고, a 필드는 A 컬럼에 ... 이런 정보)

### Run Time

> member.setTeam(team) 을 실행후 tx.commit() 하면,
